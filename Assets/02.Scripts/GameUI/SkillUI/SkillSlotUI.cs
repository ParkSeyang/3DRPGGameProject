...
public class SkillSlotUI : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler
{
    ...
    public void RefreshUI()
    {
        ...
        if (lockPanel != null)
        {
            lockPanel.gameObject.SetActive(isUnlocked == false);
        }
        
        // 버튼 활성화 상태
        levelUpButton.interactable = (isUnlocked == true && skillData.IsMaxLevel == false);
    ...
    {
        if (SkillTreeSystem.Instance != null && SkillTreeSystem.Instance.TryLevelUp(SkillID) == true)
        {
            // [수정] 자기 자신만 갱신하지 않고 트리 전체를 갱신하여 해금 연쇄 반응 반영
            SkillTreeUI.Instance?.RefreshAllSlots();
    ...
    {
        if (SkillTreeSystem.Instance != null && SkillTreeSystem.Instance.TryLevelDown(SkillID) == true)
        {
            // [수정] 레벨 다운 시에도 트리 전체를 갱신하여 즉시 잠금 처리 반영
            SkillTreeUI.Instance?.RefreshAllSlots();
            SkillTreeUI.Instance?.UpdateSPText();
        }
    }
